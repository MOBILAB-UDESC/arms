...
### 1. arms_world.launch.py
Start gazebo with a selected world.
- Args:
  - **world_name**: world file name from the [worlds](https://github.com/MOBILAB-UDESC/arms/tree/main/arms_bringup/worlds) directory. Default value is *playground* and other choices are *empty*, *mobilab*, and *pick_and_place* (last two require [mobilab_models](https://github.com/NiltonSV/gazebo_models.git)).
...
|**robot_name**|Name of the robot.|**arm**|any string|
|**test_urdf**|Launches a simple visualization of the robot in RViz if true.|false|true, false|
|**use_sim_time**|Uses simulation time and spawns the robot in Gazebo if true.|true|true, false|
...
|:---:|:---:|:---:|:---:|
|**arm**||unitree_z1||
|**base**|Mobile base platform to mount the arm on.|''|'', jackal|
|**gripper**||''||
|**prefix**||''||
|**octomap**|Whether to use octomap.|true|true, false|
|**use_sim_time**||true||
|**rviz**||true||
...
```bash
ros2 launch arms_bringup arm_world.launch.py world_name:=playground.sdf
```
```bash
ros2 launch arms_bringup arms.launch.py arm:=gen3_lite gripper:=kinova_2f_lite prefix:=kinova_ use_sim_time:=true use_camera:=true
```
```bash
ros2 launch arms_bringup arm_moveit.launch.py arm:=gen3_lite gripper:=kinova_2f_lite prefix:=kinova_ use_sim_time:=true rviz:=true
```